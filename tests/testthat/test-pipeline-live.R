...
test_that("Georgia DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Georgia DOE URL and verify HTTP 200")
})
...
test_that("Can download Georgia enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Georgia enrollment file", {
  skip_if_offline()
...
test_that("gaschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  tryCatch({
    raw <- gaschooldata:::get_raw_enr(test_year)
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- gaschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- gaschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- gaschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- gaschooldata::fetch_enr(test_year, tidy = TRUE)
